    ...
    try {
        ...
        var tsql_request=''
            ...
            if("function" === typeof tsql_request_handle) {
                // alert('=== td.operation_data 01 = ' + td.entity+ tsql_request_handle.toString())
                // console.log('=== td.operation_data 01 = ' + td.entity+ tsql_request_handle.toString())
        ...
        if(!tsql_request){
            alert('=== ERR4 !tsql_request - '+JSON.stringify(td) + tsql_request)
            return
        }

        // alert('=== TEST POINT 03 '+JSON.stringify(td.operation_data))

        if(td.operation_data) {

            for (const kk in td.operation_data) {

                const vv=td.operation_data[kk]
                tsql_request = tsql_request.replaceAll(kk+'_data',vv)
                tsql_request_read = tsql_request_read.replaceAll(kk+'_data',vv)

                console.log('=== operation_data el ',kk,vv)

            }

            // alert('=== TEST POINT 04 '+JSON.stringify(td.operation_data))
            ...
            const sql_command_type = (param_crud_type:string,param_sql:string) => {
                if(
                    ...
                    console.log("=== step_ !!! QUERY   sql_command_type =database1.query'",sql_command_type)
                        return database1.query(param_sql)
                }
                else {
                    console.log("=== step_ !!! EXECUTE sql_command_type =database1.execute'",sql_command_type)

                    try {
                            // alert('=== TEST POINT 05 '+JSON.stringify(td.operation_data))

                        const ret = database1.execute(param_sql).then((res_execute:any)=>  {
                            console.log('=== step_ res_execute 01',res_execute)
                            // alert('=== TEST POINT 09 res_execute '+JSON.stringify(res_execute))
                        }).catch((e:any)=>{
                            alert('=== TEST POINT 10 catch ERR10 '+e)
                        })

                            // console.log('=== step_ database1 ',database1)
                            // console.log('=== step_ TEST POINT 06 param_sql ', param_sql)
                            // alert('=== TEST POINT 06 '+JSON.stringify(param_sql))

                        return ret
                    }
                    catch (e:any) {
                        console.log('=== database1.execute ERR2 - ' + e + ' param_sql - ' + (param_sql) )
                        window.alert('=== database1.execute ERR2 - ' + e + ' param_sql - ' + (param_sql) )
                    }
                }
        ...
        alert("=== ERR3 exec_function_crud "+thisFile + e + ' action_to_exec = ' +JSON.stringify(action_to_exec.action.params_to_exec) )
        console.error("=== ERR3 exec_function_crud "+thisFile,e)
        console.error("=== ERR3 database1 ",database1)
    }
